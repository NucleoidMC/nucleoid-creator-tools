...
public final class MapMetadataCommand {
    public static final DynamicCommandExceptionType ENTITY_TYPE_NOT_FOUND = new DynamicCommandExceptionType(arg ->
            new TranslatableText("text.creator_tools.map.region.entity.filter.entity_type_not_found", arg)
    );
    ...
    public static final SimpleCommandExceptionType NO_REGION_READY = new SimpleCommandExceptionType(
            new TranslatableText("text.creator_tools.map.region.commit.no_region_ready")
    );
    ...
    private static int addRegion(CommandContext<ServerCommandSource> context, NbtCompound data) throws CommandSyntaxException {
        ...
        source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.region.add.success", marker)), false);
    ...
    private static int renameRegions(CommandContext<ServerCommandSource> context, RegionPredicate predicate) throws CommandSyntaxException {
        ...
        source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.region.rename.success", regions.size())), false);
    ...
    private static int getRegionBounds(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        source.sendFeedback(new TranslatableText("text.creator_tools.map.region.bounds.get.header", regions.size()).formatted(Formatting.BOLD), false);
        ...
        for (var region : regions) {
            ...
            source.sendFeedback(new TranslatableText("text.creator_tools.entry", new TranslatableText("text.creator_tools.map.region.bounds.get", minText, maxText)), false);
        }
    ...
    private static boolean executeRegionDataGet(CommandContext<ServerCommandSource> context, MapWorkspace map, WorkspaceRegion region) {
        var message = withMapPrefix(map,
                new TranslatableText("text.creator_tools.map.region.data.get", region.marker(), NBT_FORMATTER.apply(region.data()))
        );
    ...
    private static int removeRegion(CommandContext<ServerCommandSource> context, BlockPos pos) throws CommandSyntaxException {
        ...
        source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.region.remove.success", regions.size())), false);
    ...
    private static int commitRegion(CommandContext<ServerCommandSource> context, NbtCompound data) throws CommandSyntaxException {
        ...
        if (editor != null) {
            ...
            source.sendFeedback(new TranslatableText("text.creator_tools.map.region.add.success.excited", marker), false);
        }
    ...
    private static int addEntities(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (result == 0) {
            source.sendError(new TranslatableText("text.creator_tools.map.region.entity.add.error", map.getIdentifier()));
        } else {
            source.sendFeedback(new TranslatableText("text.creator_tools.map.region.entity.add.success", result, map.getIdentifier()),
                    false);
        }
    ...
    private static int removeEntities(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (result == 0) {
            source.sendError(new TranslatableText("text.creator_tools.map.region.entity.remove.error", map.getIdentifier()));
        } else {
            source.sendFeedback(new TranslatableText("text.creator_tools.map.region.entity.remove.success", result, map.getIdentifier()),
                    false);
        }
    ...
    private static int addEntityType(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (!map.addEntityType(type.getRight())) {
            source.sendError(new TranslatableText("text.creator_tools.map.region.entity.filter.type.add.already_present", type.getLeft(), map.getIdentifier()));
        } else {
            source.sendFeedback(new TranslatableText("text.creator_tools.map.region.entity.filter.type.add.success", type.getLeft(), map.getIdentifier()), false);
        }
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int removeEntityType(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (!map.removeEntityType(type.getRight())) {
            source.sendError(new TranslatableText("text.creator_tools.map.region.entity.filter.type.remove.not_present", type.getLeft(), map.getIdentifier()));
        } else {
            source.sendFeedback(new TranslatableText("text.creator_tools.map.region.entity.filter.type.remove.success", type.getLeft(), map.getIdentifier()), false);
        }
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int executeDataMerge(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.data.merge.success")), false);
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int executeDataGet(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        var source = context.getSource();
        var map = getWorkspaceForSource(context.getSource());
        source.sendFeedback(new TranslatableText("text.creator_tools.map.data.get",
    ...
    private static int executeDataGetAt(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        var map = getWorkspaceForSource(context.getSource());
        var path = NbtPathArgumentType.getNbtPath(context, "path");
        source.sendFeedback(new TranslatableText("text.creator_tools.map.data.get.at",
    ...
    private static int executeDataRemove(CommandContext<ServerCommandSource> context, @Nullable NbtPathArgumentType.NbtPath path) throws CommandSyntaxException {
        ...
        if (path == null) {
            ...
            source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.data.remove.success")),
                    false);
        } else {
            ...
            if (count == 0) {
                ...
                source.sendFeedback(withMapPrefix(map,
                        new TranslatableText("text.creator_tools.map.data.remove.at.success", path.toString())),
                        false);
            }
        }
        return Command.SINGLE_SUCCESS;
    ...
    private static int executeDataSet(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        var data = NbtCompoundArgumentType.getNbtCompound(context, "nbt");
        map.setData(data);
        source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.data.set.success")), false);
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int executeDataSetAt(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (path.put(data, element::copy) == 0) {
            ...
            map.setData(data);
            source.sendFeedback(withMapPrefix(map, new TranslatableText("text.creator_tools.map.data.set.at.success",
                            path.toString())),
                    false);
        }
        return Command.SINGLE_SUCCESS;
